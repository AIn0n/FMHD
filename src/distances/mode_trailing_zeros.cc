...
{
    int res = 0;
    for (auto b = 0; b < M_blocks; ++b) {   // iterate thru each block
        res += __builtin_popcount(bit_sketches[i][b] & bit_sketches[j][b]);
        block_t dif = ~(bit_sketches[i][b] | bit_sketches[j][b]);
        while (dif != 0) {
            const block_t t = dif & -dif;
            const int idx = block_size - 1 - __builtin_ctz(t) + block_size * b;
            res += (sketches[i][idx] == sketches[j][idx]);
            dif ^= t;
        }
    }
    return res;
